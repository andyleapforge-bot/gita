"""
Final optimized extraction with better CID mapping.
"""
...
# Enhanced CID mapping - based on analysis of common Devanagari characters
# and their relative positions in Unicode and PDF fonts
CID_MAP = {
    # Common combining marks and vowel signs
    50: '्', 51: 'ं', 52: 'ः', 53: '़',
...
CORRECTIONS = {
    ...
    'भ(cid:200)क्त योग': 'भक्ति योग',
    'ध्यान योग': 'ध्यान योग',
}

def fix_text(text):
    """Fix Hindi text by replacing CID codes."""
    if not text:
        return ""
    ...
    # Replace remaining CIDs
    def replace_cid(m):
        cid = int(m.group(1))
        return CID_MAP.get(cid, m.group(0))
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            ...
            for table in tables:
                ...
                ms = split(row[7])
                ls = split(row[9])
                zs = split(row[10]) if len(row) > 10 else []
...
if __name__ == "__main__":
    ...
    print(f"✅ Extracted {len(data)-1} Hindi shloks to {output_path}")
